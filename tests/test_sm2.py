...
@pytest.mark.parametrize("quality, expected_repetitions", [
    (0, 0),
    (1, 0),
    (2, 0),
    (3, 1),
...
@pytest.mark.parametrize("qualities, expected_repetitions, expected_interval", [
    ((1, 2), 0, 1),
    ((2, 5), 1, 1),
    ((3, 2), 0, 1),
    ((4, 2), 0, 1),
...
def test_SM2_update_params_twice(qualities, expected_repetitions, expected_interval):
    ...
    interval, repetitions, easiness = sm2(qualities[1], interval=interval, repetitions=repetitions, easiness=easiness)
...
def test_SM2_update_thrice(qualities, expected_repetitions, expected_interval):
    interval, repetitions, easiness = sm2(qualities[0], interval=0, repetitions=0, easiness=2.5)
    interval, repetitions, easiness = sm2(qualities[1], interval=interval, repetitions=repetitions, easiness=easiness)
    interval, repetitions, easiness = sm2(qualities[2], interval=interval, repetitions=repetitions, easiness=easiness)
...
def test_sm2_scheduler_compute_next_due_interval_twice(scheduler, qualities, expected_repetitions, expected_interval):
    ...
    due_timestamp_2, interval_2 = scheduler.compute_next_due_interval(attempted_at=attempted_at_2, result=qualities[1])
